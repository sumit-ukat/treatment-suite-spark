...
import type { BoardBed, BoardTask, DischargeRequestSummary, ExtensionRequestSummary, Occupant } from './board-data.js';
...
import { clientPhotos, discharge as dischargeService, extension as extensionService, tasks as taskService } from '../../services/data-access.js';
 ...
 *
 * Rendered as a separate card above the discharge workflow. Shares the same Panel/DischargeStep
 * components so the two-step visual language stays consistent.
 */
function ExtendStayCard({
  ...
  const step1Status: StepStatus = ext ? 'complete' : mode === 'form' ? 'ready' : 'waiting';
  const step2Status: StepStatus = !ext ? 'waiting' : ext.requestedBy === null ? 'ready' : 'ready';

  if (!ext && !canInitiate) return null;

  return (
    <Panel title="Extend stay" subtitle="Proposes extra days — a second person must approve before the date changes.">
      <div className="flex flex-col">
        <DischargeStep number={1} isLast={false} title="Requested" status={step1Status}>
          {!ext && mode === 'idle' ? (
            canInitiate ? (
              ...
              <p className="mt-1 text-[11px] text-[var(--color-ink-muted)]">No extension pending.</p>
            )
          ) : null}
        ...
        <DischargeStep number={2} isLast={true} title="Approved" status={step2Status}>
          {ext && canApprove && !isOwnRequest && mode === 'idle' ? (
            <div className="mt-1.5 flex items-center gap-2">
              <button
                type="button"
                disabled={busy}
            ...
            <p className="mt-1 text-[11px] text-[var(--color-ink-muted)]">Discharge date updated on approval.</p>
          )}
