-- 0038 · Wire safeguarding notes through admit_client → client_concerns
--
-- Previously, safeguarding text entered at admission was concatenated into
-- room_allocations.allocation_reason as a plain-text prefix and never reached
-- client_concerns — so the amber flag on the boards never fired and the text
-- was invisible in both popup panels.
--
-- This migration adds p_safeguarding_notes to app.admit_client / public.admit_client.
-- When provided, it inserts a 'risk' concern record into client_concerns inside the
-- same transaction, so the board flag appears immediately after admission.

drop function if exists app.admit_client(
  uuid, uuid, timestamptz, integer, text, uuid, text, text, text, text, text, boolean, text, text, text, text, boolean
);
...
begin
  ...
  -- Log safeguarding/risk concerns as a client_concern so the board flag fires immediately.
...
grant execute on function app.admit_client(
  uuid, uuid, timestamptz, integer, text, uuid, text, text, text, text, text, boolean, text, text, text, text, boolean, text
) to authenticated;
