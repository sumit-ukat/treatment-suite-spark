-- 0037 · High-risk flag on admissions
--
-- Adds high_risk boolean to admissions and threads it through app.admit_client /
-- public.admit_client so it can be set at admission time. The board data query reads it back
-- and surfaces it as hasRestrictedAlert on the Occupant type, triggering the red profile
-- highlight in the room board and treatment board panels.

alter table public.admissions
  add column if not exists high_risk boolean not null default false;

-- Drop the old 16-parameter overloads so the new 17-param version is the only one.
-- (CREATE OR REPLACE with a different parameter list creates a new overload rather than
-- replacing the old one — we explicitly drop first to avoid a ghost overload.)
...
-- PostgREST-visible wrapper (public schema) — thin pass-through, no logic.
...
as $$
  select app.admit_client(
    ...
    p_doctor_label, p_reason, p_high_risk
  );
$$;
